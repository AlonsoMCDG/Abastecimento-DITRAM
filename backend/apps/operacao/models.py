...
class GuiaAbastecimento(models.Model):
    ...
    # confundida com o veículo ou equipamento abastecido.
    ...
    # Mantidos para filtros e evolução futura. Não são obrigatórios
    # para a emissão manual da guia.
    ...
    # Pode ser um veículo cadastrado ou um recurso/equipamento descrito
    # manualmente (ex.: roçador, corote).
    ...
    quantidade_combustivel = models.DecimalField(
        max_digits=10,
        decimal_places=3
    )
    quantidade_oleo = models.DecimalField(
        max_digits=10,
        decimal_places=3,
        null=True,
        blank=True
    ...
    hodometro = models.PositiveIntegerField(
        null=True,
        blank=True,
        verbose_name="Hodômetro"
    )
    hodometro_quebrado = models.BooleanField(
        default=False,
        verbose_name="Hodômetro Quebrado"
    )
    ...
    def __str__(self):
        return (
            f"Guia #{self.id}: {self.pessoa} - "
            f"{self.veiculo_display} - {self.quantidade_combustivel}L"
        )
...
class RegistroHodometroDiario(models.Model):
    ...
    distancia_percorrida = models.DecimalField(
        max_digits=10,
        decimal_places=2,
        editable=False
    )
